...
| `STAG_B64ENC_ENVS` | *(output of `setup.sh --encode STAG`)* | Base64-encoded `.env` file. Generated by `bash scripts/setup.sh --encode STAG` — the script outputs a `.b64` file with instructions. Overlaid after every deploy to keep secrets out of the repo. |
| `STAG_B64ENC_ACME` | *(leave unset for staging)* | Base64+gzip compressed `acme.json` (Let's Encrypt certificates). Leave **unset** for staging — Traefik uses the self-signed cert from `shared/letsencrypt/` instead. Only needed for production. |
...
| `PROD_B64ENC_ENVS` | *(output of `setup.sh --encode PROD`)* | Base64-encoded production `.env`. Generated by `bash scripts/setup.sh --encode PROD`. |
| `PROD_B64ENC_ACME` | *(output of `setup.sh --encode PROD`)* | Base64+gzip `acme.json` containing your Let's Encrypt certificates. Generated by `setup.sh --encode PROD` when `acme.json` is larger than 1 KB (i.e. after Traefik has issued real certificates). Restored with `chmod 600` on every deploy. |
