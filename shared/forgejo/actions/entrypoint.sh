...
if [ ! -s "${RUNNER_FILE}" ]; then
    : "${GITBLD_AGN_SECRET:?GITBLD_AGN_SECRET is required for initial registration}"

    echo "Creating Forgejo Runner registration from shared secret..."

    # Offline registration: the UUID is derived from GITBLD_AGN_SECRET, which
    ...
    forgejo-runner \
        --config "${CONFIG_FILE}" \
        create-runner-file \
        --instance "${FORGEJO_INSTANCE_URL:-http://devopgitserv:3000}" \
        --secret "${GITBLD_AGN_SECRET}" \
        --name "${FORGEJO_RUNNER_NAME:-devopgitexec}"
else
