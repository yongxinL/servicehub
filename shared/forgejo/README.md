...
| Data persistence | `${APPS_DATA}/platform/repos` (mounted at `/data`) |
| Image tag | `DEPOT_VTAG` (default `16`) |
| Volume ownership | `1000:1000` (normally by `depotsvcinit`) |
...
> Re-registering: delete `${APPS_DATA}/platform/buildexec/.runner` (or the whole directory), re-run the Forgejo-side registration command if the secret changed, and restart `depotrunner`.
>
...
|---|---|---|
| `/data` (depotservice) | `${APPS_DATA}/platform/repos` | Repositories, `app.ini`, avatars, attachments, LFS |
...
6. Add workflow files under `.forgejo/workflows/` — see [`00-prod-deploy-services.yml`](../../.forgejo/workflows/00-prod-deploy-services.yml) (deploy), [`30-prod-backup-services.yml`](../../.forgejo/workflows/30-prod-backup-services.yml) (backup) and [`50-test-remote-access.yml`](../../.forgejo/workflows/50-test-remote-access.yml) (connectivity test).
...
rm -rf ${APPS_DATA}/platform/repos/* ${APPS_DATA}/platform/buildexec/*
docker compose up -d depotservice depotrunner
```
